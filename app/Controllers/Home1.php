        ...
        if(isset($data)){
            ...
            $user_input= $res;
                ...
                if($_FILES['avatar']['name']!=''){
                    ...
                    $avatar_folder_path='uploads/avatar';
                    $file_upload_status=false;
                     if(file_exists($avatar_folder_path)){
                         //var_dump('yes available');
                         if($avatar->move($avatar_folder_path,$new_file_name)){
                            $input_data['image_path']=$avatar_folder_path.'/'.$new_file_name;
                            $input_data['access_name']=$user_input['access_name'];
                            if(isset($user_input['id'])){
                                $input_data['id']=$user_input['id'];
                            }else{
                                $input_data['id']='';
                            }
                         ...
                         if(mkdir($avatar_folder_path,0755,true)){
                             if($avatar->move($avatar_folder_path,$new_file_name)){
                                $input_data['image_path']=$avatar_folder_path.'/'.$new_file_name;
                                $input_data['access_name']=$user_input['access_name'];
                                if(isset($user_input['id'])){
                     ...
                     if($file_upload_status==true){
                        //save all data
                        $result= $this->homemodel->add_access($input_data);
                    }else{
                       $result['status']=false;
                       $result['statuscode']=400;
                       $result['message']='avatar pic not uploaded'; 
                    }
                }else{
                    $result['status']=false;
                    $result['statuscode']=200;
                    $result['message']='select pick avatar pic'; 
                }
        
    }else{
        $result['status']=false;
        $result['statuscode']=200;
        $result['message']='please pick avatar pic'; 
...
//    }
    ...
    public function get_all_doctor_assistance(){
        $data=json_decode(file_get_contents("php://input"),true);
        $res=$this->input->getpost();
        if(isset($data)){
            $result=$this->homemodel->get_all_doctor_assistance($data);
        }else{
            $result=$this->homemodel->get_all_doctor_assistance($res);
        }
        echo json_encode($result);
    }
    ...
    public function list_doctor_assistant(){
        $data= json_decode(file_get_contents("php://input"),true);
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->list_doctor_assistant($data);
        }else{
            $result=$this->homemodel->list_doctor_assistant($res);
        }
        echo json_encode($result);
    }
    public function add_next_consulting_date(){
        $data= json_decode(file_get_contents("php://input"),true);
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->add_next_consulting_date($data);
        }else{
            $result=$this->homemodel->add_next_consulting_date($res);
        }
        echo json_encode($result);
    }
    public function add_doctor_organisation(){
        $data= json_decode(file_get_contents("php://input"),true);
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->add_doctor_organisation($data);
        }else{
            $result=$this->homemodel->add_doctor_organisation($res);
        }
        echo json_encode($result);
    }
    public function list_doctor_current_organization(){
        $data= json_decode(file_get_contents("php://input"),true);
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->list_doctor_current_organization($data);
        }else{
            $result=$this->homemodel->list_doctor_current_organization($res);
        }
        echo json_encode($result);
    }
    public function delete_organization(){
        ...
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->delete_organization($data);
        }else{
            $result=$this->homemodel->delete_organization($res);
        }
        echo json_encode($result);
    }
    public function left_organization(){
        $data= json_decode(file_get_contents("php://input"),true);
        //var_dump($data);
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->left_organization($data);
        }else{
            $result=$this->homemodel->left_organization($res);
        }
        echo json_encode($result);
    }
    public function left_organization_list(){
        $data= json_decode(file_get_contents("php://input"),true);
        //var_dump($data);
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->left_organization_list($data);
        }else{
            $result=$this->homemodel->left_organization_list($res);
        }
        echo json_encode($result);
    }
    public function add_new_doctor(){
        ...
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->add_new_doctor($data);
        }else{
            $result=$this->homemodel->add_new_doctor($res);
        }
        echo json_encode($result);
    }
    public function otp(){
    ...
    //     // var_dump($data);
    //     if(isset($data)){
    //         $result=$this->homemodel->remove_doctor($data);
    //     }else{
    //         $result=$this->homemodel->remove_doctor($res);
    //     }
    //     echo json_encode($result);
    // }
    public function list_user_medical_history(){
        $data= json_decode(file_get_contents("php://input"),true);
        $res= $this->input->getPost();
        if(isset($data)){
            $result=$this->homemodel->list_user_medical_history($data);
        }else{
            $result=$this->homemodel->list_user_medical_history($res);
        }
        echo json_encode($result);
    }
