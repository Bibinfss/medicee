...
//            $start = $startTime;
    ...
    public function get_all_doctor_assistance($data=''){
    ...
    public function get_family_members_data_by_id($data=''){
        ...
        public function list_doctor_assistant($data=''){
            if(!empty($data)){
                if(isset($data['doctor_id'])){
                    $doctor_id=$data['doctor_id'];
                }else{
                ...
                if($doctor_id!='' && $access_token!=''){
                    $check_user_data= $this->check_user_detail($doctor_id, $access_token);
                    if($check_user_data['status']){
                        $get_doctor_assistant= $this->db->table('doctor_assistant')->select('*')->where('doctor_id',$doctor_id)->where('status',1)->get()->getResultArray();
                        if(!empty($get_doctor_assistant)){
                            $response['status']=true;
                            $response['statuscode']=200;
                            $response['message']='success'; 
                            $response['data']=$get_doctor_assistant; 
                        }else{
                            $response['status']=true;
                            $response['statuscode']=200;
                            $response['message']='no data found'; 
                    ...
                    $response['status']=true;
                    $response['statuscode']=200;
                    $response['message']='doctor_id/access_token missing'; 
                }
            }else{
                $response['status']=true;
                $response['statuscode']=200;
        ...
        public function add_next_consulting_date($data=''){
            //var_dump($data);
            if(!empty($data)){
                if(isset($data['doctor_id'])){
                    $doctor_id=$data['doctor_id'];
                ...
                if($doctor_id!='' && $access_token!=''){
                    $check_user_data= $this->check_user_detail($doctor_id, $access_token);
                    if($check_user_data['status']){
                        $update_basic_data['next_follow_up_date']=$next_follow_up_date;
                        $update_consulting_data= $this->db->table('book_slot')->where('id',$booked_id)->update($update_basic_data);
                        if ($update_consulting_data) {
                            $response['status']='true';
                            $response['statuscode']='200';
        ...
        public function add_doctor_organisation($data=''){
            if($data!=''){
                if(isset($data['doctor_id'])){
                    $doctor_id=$data['doctor_id'];
                }else{
                ...
                if($doctor_id!='' && $access_token!=''){
                    $check_user_data= $this->check_user_detail($doctor_id, $access_token);
                    if($check_user_data['status']){
                        $builder = $this->db->table('doctor_current_organisation');
                    ...
                    $response['statuscode']='400';
                    $response['status']='false';
                    $response['message']='no input id/access_token found';
                }
        ...
        public function list_doctor_current_organization($data=''){
            if(!empty($data)){
                if(isset($data['doctor_id'])){
                    $doctor_id=$data['doctor_id'];
                }else{
                ...
                if($doctor_id!='' && $access_token!=''){
                    $check_user_data= $this->check_user_detail($doctor_id, $access_token);
                    if($check_user_data['status']){
                        $get_doctor_organization= $this->db->table('doctor_current_organisation')->select('*')->where('doctor_id',$doctor_id)->where('status',1)->where('working_status',1)->get()->getResultArray();
                        if(!empty($get_doctor_organization)){
                            $response['status']=true;
                            $response['statuscode']=200;
                            $response['message']='success'; 
                            $response['data']=$get_doctor_organization; 
                        }else{
                            $response['status']=true;
                            $response['statuscode']=200;
                            $response['message']='no data found'; 
        ...
        public function delete_organization($data=''){
            if($data!=''){
                ...
                //checking doctor_id
                if($doctor_id!="" && $access_token!=""){
                    //check doctor_id and access_token is valid or not
                    ...
                    if($check_user['status']==true){
                        if($id!=''){
                            ...
                            $update['status']=0;
                            //check data exist in array
                            $check_organization_id=$this->db->table('doctor_current_organisation')->select('*')->where('id',$id)->where('status',1)->get()->getRowArray();
                            if(!empty($check_organization_id)){
                                ...
                                if($data){
                                    ...
                                    $response['statuscode']=200;
                                    $response['status']=false;
                                    $response['message']="no doctor organization id found";
                                }
                            }else{
                                $response['statuscode']=200;
                                $response['status']=true;
                            ...
                            $response['statuscode']=200;
                            $response['status']=true;
                            $response['message']="Organisation Id missing";
                        }
                    }else{
                        $response['statuscode']=200;
                        $response['status']=true;
        ...
        public function left_organization($data=''){
            if($data!=''){
                if(isset($data['doctor_id'])){
                    $doctor_id=$data['doctor_id'];
                }else{
                ...
                if($doctor_id!="" && $access_token!=""){
                    ...
                    if($check_user['status']==true){
                        if($id!=''){
                            ...
                            //check data exist in array
                            $check_organization_id=$this->db->table('doctor_current_organisation')->select('*')->where('id',$id)->where('status',1)->where('working_status',1)->get()->getRowArray();
                            if(!empty($check_organization_id)){
                                $data= $this->db->table('doctor_current_organisation')->where('id',$id)->update($update);
                                if($data){
                                    $response['statuscode']=200;
        ...
        public function left_organization_list($data=''){
            if(!empty($data)){
                if(isset($data['doctor_id'])){
                    $doctor_id=$data['doctor_id'];
                }else{
                ...
                if($doctor_id!='' && $access_token!=''){
                    $check_user_data= $this->check_user_detail($doctor_id, $access_token);
                    if($check_user_data['status']){
                        $get_left_organization= $this->db->table('doctor_current_organisation')->select('*')->where('doctor_id',$doctor_id)->where('working_status',0)->get()->getResultArray();
                        if(!empty($get_left_organization)){
                            $response['status']=true;
                            $response['statuscode']=200;
                            $response['message']='success'; 
                            $response['data']=$get_left_organization; 
                        }else{
                            $response['status']=true;
                            $response['statuscode']=200;
                            $response['message']='no data found'; 
        ...
        public function add_new_doctor($data=''){
            ...
            if($check_user_data['status']){
               //check user data with country code and phone number
                $check_doctor_exist=$this->db->table('user')->select('*')->where('country_code',$country_code)->where('mobile',$phone_number)->where('access_id',1)->get()->getRowArray();
                if(empty($check_doctor_exist)){
                    // $add_doctor['datetime']=$this->current_datetime;
                    $add_doctor['country_code']=$country_code;
                    $add_doctor['mobile']=$phone_number;
                    $insert_data=$this->db->table('user')->insert($add_doctor);
                    if($insert_data){
                        $response['status']=true;
                        $response['statuscode']=200;
                        $response['message']='Success';
                    ...
                    $response['status']=false;
                    $response['statuscode']=200;
                    $response['message']='doctor already exists';
                }
            }else{
                $response['status']=false;
                $response['statuscode']=200;
    ...
    //     public function remove_doctor($data=''){
    //         // var_dump($data);
    //     if($data!=''){
    //         if(isset($data['user_id'])){
    //             $user_id=$data['user_id'];
    ...
    //                 if($id!=''){
    //                     $update['status']=0;
    //                     $check_remove_doctor=$this->db->table('user')->select('*')->where('id',$user_id)->where('status','1')->where('access_id','1')->get()->getRowArray();
    //                     if(!empty($check_remove_doctor)){
    //                         $data= $this->db->table('user')->where('id',$user_id)->update($update);
    //                         if($data){
    //                             $data= $this->db->table('employee_basic_details')->where('user_id',$user_id)->update($update);
    //                             $data= $this->db->table('employee_specialization')->where('user_id',$user_id)->update($update);
    //                             $data= $this->db->table('employee_qualification')->where('user_id',$user_id)->update($update);
    //                             $data= $this->db->table('employee_consulting_fee')->where('user_id',$user_id)->update($update);
    //                             $data= $this->db->table('employee_basic_details')->where('user_id',$user_id)->update($update);
    //                             $data= $this->db->table('doctor_current_organisation')->where('user_id',$user_id)->update($update);
    //                             $response['statuscode']=200;
    //                             $response['status']=true;
    //                             $response['message']="Deleted Successfully";
    //                         }else{
    ...
    //     return $response;
    // }
        public function list_user_medical_history($data=''){
            if(!empty($data)){
                if(isset($data['user_id'])){
                    $user_id=$data['user_id'];
                }else{
                ...
                if($user_id!='' && $access_token!='' ){
                    ...
                    if($check_user['status']){
                        ...
                        $medical_history= $this->db->table('medical_history')->select('medical_history.id,medical_history.file_path,medical_history.description,medical_history.added_datetime')->where('medical_history.user_id',$user_id)->where('medical_history.family_member_id',$family_member_id)->get()->getResultArray();
